...
void Profile::setBrightnessWithNoise(float minBr, float maxBr)
{
	...
	if (maxBr + 4 * this->getStdDev() < 0)
	{
		throw "Max brightness must be positive";
	}

	//TODO придумать проверку предельных значений ярокости с учётом шума
	...
	std::normal_distribution<float> norm(this->getMean(), this->getStdDev());
...
void Profile::setNoiseParams(float mean, float stdDev)
{
	this->noiseMean = mean;
...
float Profile::getMean() const
{
	return noiseMean;
}

float Profile::getStdDev() const
{
	return noiseStdDev;
}
